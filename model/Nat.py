...
class NeighborhoodAttention(
    ...
    def attention(self, x):
        ...
        # selected_bias = torch.gather(self.relative_bias.view(-1, 4), 0, self.bias_idx.view(-1, 1).expand(-1, 4))
        # selected_bias = selected_bias.view(self.bias_idx.shape[0], self.bias_idx.shape[1], -1)
        # attn = attn + selected_bias.permute(2, 0, 1).unsqueeze(2)
...
def test():
    print('it is cpu')
    model = NATLayer((32, 32), 128, 4)
    img = torch.rand(4, 128, 32, 32)
...
def test_cuda():
    print('it is cuda')
    model = NATLayer((32, 32), 128, 4).cuda()
    img = torch.rand(4, 128, 32, 32).cuda()
    print(model(img).shape)
    print('success')
    print('cuda_success\n')
